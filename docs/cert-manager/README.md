...
cert-manager can use different certificate issuing services to automate the lifecycle of TLS certificates. For the purpose of demonstration, Let's Encrypt service uses the Automated Certificate Management Environment (ACME) Certificate Authority server to issue certificates.
...
A secret containing the service principal password should be created on Kubernetes to facilitate presenting the challenge to Azure DNS. You can create the secret with the following command:
...
To configure the issuer, substitute the capital cased variables with the values from the previous script as well as a valid email address. A sample yaml has been provided to configure: [lets-encrypt-prod.yaml](../../src/yaml/cert-manager-config/lets-encrypt-prod.yaml)
...
```bash
kubectl -n cert-manager apply -f src/yaml/cert-manager-config/lets-encrypt-prod.yaml
```
