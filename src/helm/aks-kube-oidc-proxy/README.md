...
This is a `helm` chart that installs [`kube-oidc-proxy`](https://github.com/jetstack/kube-oidc-proxy/) into an AKS Cluster.
This helm chart cannot be installed out of the box without providing some specific
configuration to the [values.yaml](./values.yaml) file

This helm chart is based on example configuration provided in `kube-oidc-proxy`
[repository](https://github.com/retaildevcrews/aks-kube-oidc-proxy).
...
oidc:
  clientId: my-client
  issuerUrl: https://accounts.mydomain.com
  usernameClaim: email
```
...
clients. Example of exposing via a Contour ingress controller.
